# Thread Object which generates random entries and writes to a simulation log to test the LogHandler
...
class EntryGenerator(Thread):
    ...
    def __init__(self,logPath,rate):
        ...
        # Set it to False to stop the generation loop and end the thread (stop())
        self.running = True
        # Set it to False to stop generating whithout ending the thread
        self.generating = True
        self.ips = ["::1","192.168.0.110","127.0.0.1","60.242.26.14"]
        self.methods = ["GET","GET","GET","POST","POST","HEAD","PUT","DELETE"]
        self.sections = ["/img","/captcha","/audio","/css","/foo","/foo","/foo","/bar"]
    ...
    def generate_entry(self,entryTime):
        ...
        # Randomize section choose first part as random and then choose if it's a html file or if there will be another section
        section = random.choice(self.sections) + random.choice([".html",random.choice(self.sections)+".html"])
