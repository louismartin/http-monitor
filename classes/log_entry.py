...
class LogEntry:
    ...
    def __init__(self,entryLine):
        ...
        example: 127.0.0.1 - - [30/May/2015:14:13:09 +1000] "GET /icons/blank.gif HTTP/1.1" 403 301"
        ...
        try:
            ...
            # 3rd item is the time, we don't take the zone into account though (4th item)
            timeString = entryValues[3].strip("[]") 
            self.time = datetime.strptime(timeString, "%d/%b/%Y:%H:%M:%S")
            
            self.method = entryValues[5].strip("\"")

            # Split path on / to get different sections: "/icons/blank.gif" will yield ['', 'icons', 'blank.gif']
            ...
            if sizeString == "-": # if size is not provided, there is a dash instead
                self.size = 0
            else:
            ...
            self.parsed = False
            # Set the time to now so that it does not stop the LogHandler reading process
            self.time = datetime.now()


    def __str__(self):
        """toString method, provides a readable String representation of the attributes of the object""" 
        return self.ip+" "+str(self.time)+" "+self.method+" "+self.section+" "+str(self.size)
