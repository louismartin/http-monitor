...
class TestLogEntry(unittest.TestCase):
    ...
    def test_log_entry(self):
        ...
        now = datetime.now()
        now = datetime(now.year, now.month, now.day, now.hour, now.minute, now.second)    #truncate current datetime to remove microseconds (for the test to succeed)
        entryLine = '127.0.0.1 - - [%s +1000] "GET /icons/blank.gif HTTP/1.1" 403 301' % now.strftime("%d/%b/%Y:%H:%M:%S")
...
class TestLogHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.logHandler = LogHandler(self.logPath, self.refreshPeriod, self.alertThreshold, self.monitorDuration)
    ...
    def test_add_entry(self):
        ...
        # Let's put an entry that is not formatted to check if it's correctly dropped
    ...
    def test_several_reads(self):
        """Test the behaviour of LogHandler when it reads several times in a row"""
        print("********************************")
        print("test_several_reads()")
        self.logHandler.read()
        # Check that only the two most recent inital entries have been processed
        self.assertEqual(len(self.logHandler.log),2)
        # Add a new entry
    ...
    def test_run(self):
        ...
        # Check delta between two log reads is within a 10% error margin from the specified refreshPeriod
        self.assertTrue(abs((delta-self.refreshPeriod)/self.refreshPeriod)<0.1) 
    ...
    def test_drop_old_entries(self):
        ...
        oldEntryString = self.entryGenerator.generate_entry(datetime.now() - timedelta(hours=10))
        oldEntry = LogEntry(oldEntryString)
    ...
    def test_end_alert(self):
        ...
        # Set time frame of entries taken into account to 1 second to test faster
    ...
    def test_summary(self):
        ...
        self.logHandler.monitorDuration = 2
        now = datetime.now()
        # Truncate current datetime to remove microseconds (for the test to succeed)
        now = datetime(now.year, now.month, now.day, now.hour, now.minute, now.second)
        entries = ('127.0.0.1 - - [%s +1000] "GET /icons/blank.gif HTTP/1.1" 200 100\n' % now.strftime("%d/%b/%Y:%H:%M:%S")+
            '289.8.42.1 - - [%s +1000] "POST /index.html HTTP/1.1" 200 1000\n' % now.strftime("%d/%b/%Y:%H:%M:%S")+
            '127.0.0.1 - - [%s +1000] "GET /icons/blank.gif HTTP/1.1" 200 900\n' % now.strftime("%d/%b/%Y:%H:%M:%S")+
            '289.8.42.1 - - [%s +1000] "GET /css/display.css HTTP/1.1" 403 4000\n' % now.strftime("%d/%b/%Y:%H:%M:%S")+
            '127.0.0.1 - - [%s +1000] "GET /index.php HTTP/1.1" 404 1000\n' % now.strftime("%d/%b/%Y:%H:%M:%S")+
            '289.8.42.1 - - [%s +1000] "POST /icons/blank.gif HTTP/1.1" 200 9000\n' % now.strftime("%d/%b/%Y:%H:%M:%S")+
            '127.0.0.1 - - [%s +1000] "GET /icons/blank.gif HTTP/1.1" 403 4000\n' % now.strftime("%d/%b/%Y:%H:%M:%S"))
        ...
        self.assertEqual(self.logHandler.sections,{"icons":4,"root":2,"css":1})
        self.assertEqual(self.logHandler.ips,{"127.0.0.1":4,"289.8.42.1":3})
